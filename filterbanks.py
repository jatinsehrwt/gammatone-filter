...
class FilterBank(object):
    ...
    fs = sample rate
    N = number of frequency channels / subbands (excluding high-&low-pass which
        are added for perfect reconstruction)
    ...
    def __init__(self, leny, fs, N, low_lim, high_lim):
        ...
        self.high_lim, self.freqs, self.nfreqs = self.check_limits(
            leny, fs, high_lim)
    ...
    def generate_subbands(self, signal):
        ...
        if np.remainder(signal_length, 2) == 0:  # even length
            fft_filts = np.concatenate(
                [self.filters, np.flipud(self.filters[1:filt_length - 1, :])])
        else:  # odd length
            fft_filts = np.concatenate(
                [self.filters, np.flipud(self.filters[1:filt_length, :])])
        ...
        self.subbands = np.transpose(
            np.real(np.fft.ifft(np.transpose(fft_subbands))))


class EqualRectangularBandwidth(FilterBank):
    def __init__(self, leny, fs, N, low_lim, high_lim):
        super(EqualRectangularBandwidth, self).__init__(
            leny, fs, N, low_lim, high_lim)
        ...
        self.filters = self.make_filters(
            self.N, self.nfreqs, self.freqs, self.cutoffs)
    ...
    def make_filters(self, N, nfreqs, freqs, cutoffs):
        ...
        filters[:h_ind + 1, 0] = np.sqrt(1 -
                                         np.power(filters[:h_ind + 1, 1], 2))
        # highpass filter goes down to peak of last cos filter
        l_ind = np.min(np.where(freqs > cutoffs[N]))
        filters[l_ind:nfreqs + 1, N +
                1] = np.sqrt(1 - np.power(filters[l_ind:nfreqs + 1, N], 2))
        return filters
    ...
    def make_filters(self, N, nfreqs, freqs, cutoffs):
        ...
        for k in range(N):
            ...
            cos_filts[l_ind:h_ind + 1,
                      k] = np.cos((freqs[l_ind:h_ind + 1] - avg) / rnge * np.pi)
        # add lowpass and highpass to get perfect reconstruction
        filters = np.zeros([nfreqs + 1, N + 2])
        filters[:, 1:N + 1] = cos_filts
        # lowpass filter goes up to peak of first cos filter
